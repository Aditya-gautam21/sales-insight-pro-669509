...
const llm = new Llm({ provider: process.env.LLM_PROVIDER as LlmProvider || 'google' });

export const generateResearch = async (input: any) => {
...
const generateSingleCompanyInsights = async (input: any) => {
  const { companyName, industry, targetRole, productService, tone } = input;
...
const findBulkLeads = async (input: any) => {
    ...
    for (const companyName of companyNames) {
      ...
      await prisma.researchHistory.create({
        data: {
          ...
          leadScore: insights.leadScore.score,
          insights: insights as any,
          input: { ...input, companyName } as any
        }
      });
    }
...
const calculateAuditedScore = (companyName: string, industry: string, breakdown: any) => {
...
export const createTemplate = async (name: string, input: any) => {
